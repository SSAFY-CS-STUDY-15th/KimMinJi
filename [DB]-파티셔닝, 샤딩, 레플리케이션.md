...
<img src="assets/PRA3-MySQL-replication.png" alt="MySQL 마스터-슬레이브 서버 구조" width="80%"/>

#### 1. 마스터: 바이너리 로그(Binary Log) 기록

원본 서버에서 발생한 모든 변경 사항(INSERT, UPDATE, DELETE 등)은 바이너리 로그에 기록됩니다.

슬레이브가 연결되면, 소스 서버는 Binlog Dump라는 전용 쓰레드를 생성합니다. 이 쓰레드는 바이너리 로그를 읽어 슬레이브 I/O 쓰레드로 데이터를 보내주는 역할을 합니다.

#### 2. 슬레이브: I/O 쓰레드

슬레이브 서버가 시작되면 소스 서버에 접속을 요청하고 I/O 쓰레드를 실행합니다.

I/O 쓰레드는 마스터 서버에 접속하여 마지막으로 읽었던 바이너리 로그의 위치 이후부터 데이터를 보내달라고 요청합니다.

소스로부터 받아온 데이터는 슬레이브 서버의 디스크에 릴레이 로그라는 파일로 저장됩니다.

#### 3. 슬레이브: SQL 쓰레드

SQL 쓰레드는 릴레이 로그에 기록된 변경 사항을 실제 레플리카 데이터베이스에 반영합니다.

SQL 쓰레드는 릴레이 로그를 읽어 하나씩 실행하고 실제 슬레이브 DB에 반영합니다. 실행이 완료된 릴레이 로그는 용량 확보를 위해 자동으로 삭제됩니다.

최근 MySQL 버전에서는 성능 향상을 위해 여러 개의 SQL 쓰레드를 사용하는 병렬 복제 기능을 지원합니다.
