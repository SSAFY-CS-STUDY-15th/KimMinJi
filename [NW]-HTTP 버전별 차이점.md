...
다양한 문제점들이 있었지만 가장 큰 문제점은 HOL(Head Of Line) Blocking 문제였습니다. <br/>HOL Blocking 이란 서버가 항상 요청받은 순서대로 응답하는 것으로 한 번에 한개의 응답만 처리할 수 있었기 때문이 발생했습니다. 만약 아주 작은 스타일시트 리소스를 받아오기 전에 고용량 이미지에 대한 요청이 들어갔다면 고용량 이미지가 다 올 때까지 TCP 버퍼에서 대기해야 했습니다.<br/>HTTP 1.1에서는 물리적인 TCP를 여러개 두어 이 문제를 임시적으로나마 해결하려고 했습니다.
...
## Q&A
### 1. 만약 클라이언트에서 Http 2.0으로 요청을 보내면 1.1 서버는 어떻게 응답하나요?
HTTP는 기본적으로 이전 버전과의 호환성을 염두에두고 발전했기 때문에 HTTP/2 서버의 경우 클라이언트가 HTTP/1.1로 요청을 보내면 HTTP/1.1로 응답하게 됩니다.
말씀하신 클라이언트가 HTTP/2로 HTTP/1.1 서버에 요청을 보내게 됐을 때도 기본적으로 다운그레이드 됩니다. HTTPS의 경우 ALPN을 통해 지원가능한 프로토콜을 서버와 클라이언트가 교환하게 되는데 이 때 서버가 HTTP/1.1만 지원하는 것을 알게되고 HTTP/1.1 프로토콜로 통신하게 됩니다.

### 2. 현재 코드에서 slow.css의 전송이 resource3.css의 전송보다 항상 선행되나요?
기본적으로는 브라우저의 HTML 파서가 위에서부터 아래로 요청하게됩니다. 단 프리로드 스캐너라는게 있어서 요청해야할 리소스 목록을 쭉 스캔하게 되는데 이 때 우선순위에 따라 순서가 변경될 수 있습니다.
(HTML 문서 자체, 안의 CSS, 폰트 등이 가장 우선순위가 높고 그 다음이 js, 리소스, 마지막이 이미지, 비디오, 비동기 스크립트 라고 합니다.)
slow.css 와 resource.css 모두 다 같은 head 안의 css 파일들이고 순서상 resource1 -> resource2 -> slow1 -> slow2... 순으로 요청했기 때문에 resource3.css 부터는 확실하게 HOL Blocking 현상이 발생한다고 보시면됩니다.
